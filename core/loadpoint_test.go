...
func TestUpdatePowerZero(t *testing.T) {
	...
	for _, tc := range tc {
		...
		lp.mode = tc.mode
		lp.Update(0, 0) // false,sitePower false,0
	...
	for _, status := range []api.ChargeStatus{api.StatusB, api.StatusC} {
		for _, tc := range tc {
			...
			for step, se := range tc.series {
				...
				current := lp.pvMaxCurrent(api.ModePV, se.site, se.site)
...
func TestPVHysteresisForStatusOtherThanC(t *testing.T) {
	...
	current := lp.pvMaxCurrent(api.ModePV, sitePower, sitePower)
...
func TestPVHysteresisAfterPhaseSwitch(t *testing.T) {
	...
	for _, tc := range tc {
		...
		for step, se := range tc.series {
			clck.Set(start.Add(se.delay))
			assert.Equal(t, se.current, lp.pvMaxCurrent(api.ModePV, se.site, se.site), step)
		}
